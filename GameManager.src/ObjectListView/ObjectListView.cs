 ...
 *
 * Change log
 *                    Added code in BuildList() to fix random list scrolling when saving a game
 *                    Added code in HandleColumnClick() to respect a column's default sort order
 *                    Added some code in DoSort() and PostProcessRows() to allow sorting on non-groupable columns 
 *                    Commented out the HandleChar function to get rid of the ding sound when typing.
...
// ReSharper restore FieldCanBeMadeReadOnly.Global
        ...
        public virtual int TopItemIndex {
            ...
            set {
                ...
                if (this.View != View.Details || newTopIndex < 0) return;

                int scrollVectorY = GetItemRect(newTopIndex).Y - GetItemRect(this.TopItem.Index).Y;
                LowLevelScroll(0, scrollVectorY);
            }
        }
...
#pragma warning restore 168
        ...
        public virtual void BuildList(bool shouldPreserveState) {
            ...
            Point oldScrollPos = this.LowLevelScrollPosition;
            ...
            Point currentScrollPos = this.LowLevelScrollPosition;
            ...
            if (shouldPreserveState) {
                ...
                if (this.ShowGroups && this.LastSortColumn != null && this.LastSortColumn.Groupable) {
                    if (oldScrollPos != currentScrollPos) {
                        this.LowLevelScroll(oldScrollPos.X - currentScrollPos.X, oldScrollPos.Y - currentScrollPos.Y);
                    }
                }
        ...
        public virtual void ChangeToFilteredColumns(View view) {
            ...
            //Limit the list to 1 item to prevent the list from lagging when Clear is called
            this.ListFilter = new TailFilter(1);
...
// ReSharper restore PossibleMultipleEnumeration
        ...
        public bool RestoreState(byte[] state) {
            using (MemoryStream ms = new MemoryStream(state)) {
                ...
                //If the list was missing the 'language' column when the list state was saved, the list state will be invalid
                //This is corrected manually by inserting the correct size and display order for the language column into the list state
                if (olvState.NumberOfColumns == 15 && AllColumns.Count == 16) {
                    olvState.ColumnWidths.Insert(5, 92);
                    olvState.ColumnIsVisible.Insert(5, false);
                    olvState.ColumnDisplayIndicies.Insert(5, 5);
                    olvState.NumberOfColumns = 16;

                    if (olvState.SortColumn >= 5) {
                        olvState.SortColumn++;
                    }
                }
...
// ReSharper restore FieldCanBeMadeReadOnly.Global
        ...
        protected virtual void HandleColumnClick(object sender, ColumnClickEventArgs e) {
            ...
            OLVColumn column = this.GetColumn(e.Column);

            //If the list is going from ungrouped to grouped, the list must be rebuilt after sorting
            bool rebuildRequired = this.ShowGroups && column.Groupable && LastSortColumn != null && !LastSortColumn.Groupable;
            ...
            if (this.PrimarySortColumn != null && e.Column == this.PrimarySortColumn.Index)
                ...
                this.PrimarySortOrder = column.DefaultSortOrder == SortOrder.None ? SortOrder.Ascending : column.DefaultSortOrder;
